...
class GameModeUtils @Inject constructor(private val context: Context) {
    ...
    fun setGameModeFor(packageName: String, systemSettings: SystemSettings, mode: Int): UserGame {
        val existing = systemSettings.userGames.firstOrNull { it.packageName == packageName }
        val data = UserGame(packageName, mode, existing?.fpsCap ?: UserGame.FPS_CAP_DEFAULT)
    ...
    fun setFpsCapFor(packageName: String, systemSettings: SystemSettings, fpsCap: Int): UserGame {
        val existing = systemSettings.userGames.firstOrNull { it.packageName == packageName }
        val data = UserGame(packageName, existing?.mode ?: defaultPreferredMode, fpsCap)
        systemSettings.userGames = systemSettings.userGames
            .filter { x -> x.packageName != packageName }
            .toMutableList()
            .apply { add(data) }
