...
class PerAppSettingsFragment : Hilt_PerAppSettingsFragment(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        findPreference<ListPreference>(PREF_FPS_CAP)?.apply {
            currentConfig?.fpsCap?.let { value = it.toString() }
            onPreferenceChangeListener = this@PerAppSettingsFragment
        }
    ...
    override fun onPreferenceChange(preference: Preference, newValue: Any?): Boolean {
        ...
        when (preference.key) {
            ...
            PREF_FPS_CAP -> {
                val newFps = (newValue as String).toIntOrNull() ?: UserGame.FPS_CAP_DEFAULT
                gameModeUtils.setFpsCapFor(gameInfo.packageName, settings, newFps)
                return true
            }
            PREF_USE_ANGLE -> {
    ...
    companion object {
        ...
        const val PREF_FPS_CAP = "per_app_fps_cap"
